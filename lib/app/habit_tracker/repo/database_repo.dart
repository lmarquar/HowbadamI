...
import 'package:howbadami/core/constants/storage/storage_keys.dart';
...
class DatabaseRepository {
  ...
  DatabaseRepository(this._sharedPrefService);

  Future<Either<Failure, UserHabitEntity>> getUserHabit() async {
    try {
      String? jsonString = await _sharedPrefService.getData(
        path: StorageKeys.userHabit,
      );
      if (jsonString == null || jsonString.isEmpty) {
        return Right(UserHabitModel.createEntity());
      }
      UserHabitEntity userHabitEntity = UserHabitModel.fromStringToEntity(
  ...
  Future<Either<Failure, UserHabitEntity>> setUserHabit({
    required UserHabitEntity entity,
  }) async {
    try {
