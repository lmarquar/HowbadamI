...
import 'package:howbadami/core/constants/storage/user_habit_keys.dart';
...
class UserHabitProvider extends ChangeNotifier {
  ...
  Future<void> eitherFailureOrUserHabitDateUpdate({
    ...
    userHabit!.progress = _updateProgressFromDateTime(
      userHabit!.progress,
      date,
    );
  ...
  Map<String, Map<String, dynamic>> _updateProgressFromDateTime(
    ...
    String formattedDate = DateFormat(
      AppFormatters.dateFormatyyyyMMdd,
    ).format(date);
    String formattedTime = DateFormat(
      AppFormatters.dateFormatHHmmss,
    ).format(DateTime.now());
    progress.update(
      formattedDate,
